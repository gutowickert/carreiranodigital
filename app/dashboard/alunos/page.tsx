  ...
  return (
    <div style={{ padding: '24px', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--text)' }}>Alunos</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Total de alunos', val: alunos.length, cor: 'var(--text)' },
          { label: 'Matrículas', val: totalMatriculas, cor: 'var(--text)' },
          { label: 'Fizeram 2+ cursos', val: recompradores, cor: 'var(--accent-soft)' },
          { label: 'LTV total', val: fmt(totalLtv), cor: 'var(--green)' },
        ].map(item => (
      ...
      {novoAluno && (
        <div style={{ ...card, padding: '24px', marginBottom: '24px' }}>
          <div style={{ fontSize: '15px', fontWeight: '600', color: 'var(--text)', marginBottom: '16px' }}>Cadastrar novo aluno</div>
          <form onSubmit={salvarAluno}>
            ...
            {mensagem && <p style={{ marginTop: '8px', fontSize: '13px', color: 'var(--red)' }}>{mensagem}</p>}
          </form>
        </div>
      )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '12px 24px', borderBottom: '1px solid var(--border)' }}>
              <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{alunosFiltrados.length} aluno{alunosFiltrados.length !== 1 ? 's' : ''}</span>
            </div>
            {carregando ? <p style={{ padding: '24px', fontSize: '14px', color: 'var(--text-faint)' }}>Carregando...</p>
              : alunosFiltrados.length === 0 ? <p style={{ padding: '24px', fontSize: '14px', color: 'var(--text-faint)' }}>Nenhum aluno ainda.</p>
              : alunosFiltrados.map(a => (
                <div key={a.id} onClick={() => setSelecionado(a)}
                  style={{ padding: '14px 24px', borderBottom: '1px solid var(--border)', cursor: 'pointer', backgroundColor: selecionado?.id === a.id ? 'var(--surface-sel)' : 'transparent' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '14px', fontWeight: '500', color: 'var(--text)' }}>{a.nome}</span>
                        {(a.matriculas?.length || 0) > 1 && (
                          <span style={{ fontSize: '11px', backgroundColor: 'var(--accent-bg)', color: 'var(--accent-soft)', padding: '2px 8px', borderRadius: '20px' }}>recorrente</span>
                        )}
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--green)' }}>{fmt(a.ltv)}</div>
                      <div style={{ fontSize: '12px', color: 'var(--text-faint)' }}>{a.matriculas?.length || 0} curso{(a.matriculas?.length || 0) !== 1 ? 's' : ''}</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '12px' }}>
                  <div style={{ backgroundColor: 'var(--green-bg)', borderRadius: '8px', padding: '10px', textAlign: 'center' }}>
                    <div style={{ fontSize: '14px', fontWeight: '700', color: 'var(--green)' }}>{fmt(selecionado.ltv)}</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-faint)' }}>LTV</div>
                  </div>
                  <div style={{ backgroundColor: 'var(--accent-bg)', borderRadius: '8px', padding: '10px', textAlign: 'center' }}>
                    <div style={{ fontSize: '14px', fontWeight: '700', color: 'var(--accent-soft)' }}>{selecionado.matriculas?.length || 0}</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-faint)' }}>Cursos</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: '16px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Cursos</span>
                  <button onClick={() => setNovaMatricula(!novaMatricula)} style={{ fontSize: '12px', color: 'var(--accent-soft)', background: 'none', border: 'none', cursor: 'pointer' }}>+ Matricular</button>
                </div>

                {novaMatricula && (
                  <form onSubmit={salvarMatricula} style={{ backgroundColor: 'var(--surface-2)', borderRadius: '10px', padding: '12px', marginBottom: '12px' }}>
                ...
                {!selecionado.matriculas || selecionado.matriculas.length === 0
                  ? <p style={{ fontSize: '12px', color: 'var(--text-faint)' }}>Nenhuma matrícula.</p>
                  : selecionado.matriculas.map(m => (
                    <div key={m.id} style={{ backgroundColor: 'var(--surface-2)', borderRadius: '8px', padding: '10px', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <div>
                          <div style={{ fontSize: '13px', fontWeight: '500', color: 'var(--text)' }}>{m.turmas?.produtos?.nome}</div>
                          <div style={{ fontSize: '11px', color: 'var(--text-faint)' }}>
                            {m.turmas?.cidades?.nome}{m.turmas?.data_inicio ? ` · ${new Date(m.turmas.data_inicio + 'T12:00:00').toLocaleDateString('pt-BR')}` : ''}
                          </div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--green)' }}>{fmt(m.valor_pago)}</div>
                          <span style={{ fontSize: '11px', backgroundColor: m.status === 'ativa' ? 'var(--blue-bg)' : m.status === 'concluida' ? 'var(--green-bg)' : 'var(--red-bg)', color: m.status === 'ativa' ? 'var(--blue)' : m.status === 'concluida' ? 'var(--green-strong)' : 'var(--red)', padding: '2px 6px', borderRadius: '20px' }}>
                            {m.status}
                          </span>
                        </div>
                      </div>
