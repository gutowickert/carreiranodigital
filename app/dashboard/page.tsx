...
const statusCor: Record<string, string> = {
  planejada: 'bg-gray-800 text-gray-400',
  em_vendas: 'bg-blue-900 text-blue-400',
  confirmada: 'bg-green-900 text-green-400',
  realizada: 'bg-purple-900 text-purple-400',
...
export default function Turmas() {
  ...
  async function carregarTurmas() {
    const { data } = await supabase.from('turmas').select('*, produtos(nome), cidades(nome), salas(nome)').order('data_inicio', { ascending: false })
    ...
    if (data && data.length > 0) {
      ...
      data.forEach(mod => {
        for (let i = 0; i < mod.duracao_dias; i++) {
          dias.push({ data: '', horario_inicio: '08:00', horario_fim: '18:00', modulo_id: mod.id, modulo_nome: mod.nome })
        }
      })
      setDiasAula(dias)
    } else {
  ...
  async function handleSalvar(e: React.FormEvent) {
    ...
    if (diasAula.length > 0) {
      await supabase.from('turma_datas').insert(
        diasAula.map((d, i) => ({ turma_id: turma.id, modulo_id: d.modulo_id || null, data: d.data, horario_inicio: d.horario_inicio, horario_fim: d.horario_fim, ordem: i + 1 }))
      )
    }
    ...
    if (templates && templates.length > 0 && dataInicio) {
      ...
      await supabase.from('tarefas').insert(templates.map(t => {
        const dp = new Date(inicio)
        dp.setDate(dp.getDate() + t.dias_relativo)
    ...
    setAbrirForm(false)
    setProdutoId(''); setCidadeId(''); setSalaId(''); setProfessorId('')
    setProfPorModulo({}); setDiasAula([]); setModulos([])
    setDeslocProf('0'); setDeslocEquipe('0'); setCodigoTurma('')
    carregarTurmas()
    setSalvando(false)
  }
  ...
  const diasPorModulo = modulos.length > 0
    ? modulos.map(mod => ({ modulo: mod, dias: diasAula.map((d, i) => ({ ...d, index: i })).filter(d => d.modulo_id === mod.id) }))
    : null
    ...
    <div className="p-6 min-h-screen">
      ...
      {abrirForm && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-300 mb-4">Nova turma</h2>
          <form onSubmit={handleSalvar} className="space-y-4">

            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1">Código da turma</label>
              ...
              <p className="text-xs text-gray-600 mt-1">Identificador único — vincula leads do formulário a esta turma</p>
            </div>

            <div className="flex gap-3">
              <select value={produtoId} onChange={e => setProdutoId(e.target.value)} required
                className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white flex-1 focus:outline-none focus:border-purple-500">
            ...
            <div className="flex gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Preço de venda R$</label>
                <input value={preco} onChange={e => setPreco(e.target.value)} type="number" required
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white w-36 focus:outline-none focus:border-purple-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Vagas disponíveis</label>
                <input value={vagas} onChange={e => setVagas(e.target.value)} type="number" required
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white w-36 focus:outline-none focus:border-purple-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Mínimo para não cancelar</label>
            ...
            {produtoSelecionado && modulos.length === 0 && (
              <div>
                ...
                <select value={professorId} onChange={e => setProfessorId(e.target.value)}
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white w-full focus:outline-none focus:border-purple-500">
                  <option value="">Selecione o professor</option>
                  {professores.map(p => <option key={p.id} value={p.id}>{p.nome} — R$ {p.diaria_reais}/dia (total: R$ {p.diaria_reais * produtoSelecionado.duracao_dias})</option>)}
                </select>
              </div>
            )}
                        ...
                        <div key={modulo.id}>
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className="text-xs bg-purple-900 text-purple-400 px-2 py-1 rounded-full">Módulo {modulo.ordem}</span>
                              <span className="text-sm font-medium text-white">{modulo.nome}</span>
                              <span className="text-xs text-gray-500">({modulo.duracao_dias} dias)</span>
                            </div>
                            <select value={profPorModulo[modulo.id] || ''} onChange={e => setProfPorModulo(prev => ({ ...prev, [modulo.id]: e.target.value }))}
                              className="bg-gray-700 border border-gray-600 rounded-lg px-2 py-1 text-xs text-white focus:outline-none max-w-xs">
                              <option value="">Selecionar professor</option>
                              {professores.map(p => <option key={p.id} value={p.id}>{p.nome} — R$ {p.diaria_reais * modulo.duracao_dias}</option>)}
                            </select>
                          </div>
                          <div className="space-y-2">
                            {dias.map((dia, i) => (
                              <div key={dia.index} className="flex gap-3 items-center">
                                <span className="text-xs text-gray-500 w-12">Dia {i + 1}</span>
                                <input type="date" value={dia.data} onChange={e => atualizarDia(dia.index, 'data', e.target.value)} required
                                  className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none" />
                                <input type="time" value={dia.horario_inicio} onChange={e => atualizarDia(dia.index, 'horario_inicio', e.target.value)}
                                  className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white w-28 focus:outline-none" />
                                <span className="text-xs text-gray-500">até</span>
                                <input type="time" value={dia.horario_fim} onChange={e => atualizarDia(dia.index, 'horario_fim', e.target.value)}
                                  className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white w-28 focus:outline-none" />
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="space-y-2">
                      {diasAula.map((dia, i) => (
                        <div key={i} className="flex gap-3 items-center">
                          <span className="text-xs text-gray-500 w-12">Dia {i + 1}</span>
                          <input type="date" value={dia.data} onChange={e => atualizarDia(i, 'data', e.target.value)} required
                            className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none" />
                          <input type="time" value={dia.horario_inicio} onChange={e => atualizarDia(i, 'horario_inicio', e.target.value)}
                            className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white w-28 focus:outline-none" />
                          <span className="text-xs text-gray-500">até</span>
                          <input type="time" value={dia.horario_fim} onChange={e => atualizarDia(i, 'horario_fim', e.target.value)}
                            className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white w-28 focus:outline-none" />
                        </div>
                      ))}
                    </div>
            ...
            {isExterna && (
              <div className="flex gap-3 bg-orange-950 border border-orange-900 p-3 rounded-lg">
                <div className="flex-1">
                  <label className="text-xs text-orange-400 font-medium block mb-1">Deslocamento professor R$</label>
                  <input value={deslocProf} onChange={e => setDeslocProf(e.target.value)} type="number"
                    className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white w-full focus:outline-none" />
                </div>
                <div className="flex-1">
                  <label className="text-xs text-orange-400 font-medium block mb-1">Deslocamento equipe R$</label>
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button type="submit" disabled={salvando}
                className="bg-purple-600 hover:bg-purple-700 text-white rounded-lg px-6 py-2 text-sm font-medium transition-colors disabled:opacity-50">
                {salvando ? 'Abrindo...' : 'Abrir turma'}
              </button>
            </div>
            {mensagem && <p className={`text-sm ${mensagem.includes('Erro') ? 'text-red-400' : 'text-green-400'}`}>{mensagem}</p>}
          </form>
        </div>
      )}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-800">
          <h2 className="text-sm font-medium text-gray-400">Turmas abertas</h2>
        </div>
        {turmas.length === 0 ? (
          <p className="text-sm text-gray-500 p-6">Nenhuma turma aberta ainda.</p>
        ) : (
            ...
            <tbody>
              {turmas.map(t => (
                <tr key={t.id}
                  ...
                  <td className="px-6 py-3 text-sm text-white font-medium">{t.produtos?.nome}</td>
                  <td className="px-6 py-3 text-sm text-gray-400">{t.cidades?.nome}</td>
                  <td className="px-6 py-3 text-sm text-gray-500 font-mono">{t.codigo || '—'}</td>
                  <td className="px-6 py-3 text-sm text-gray-400">{new Date(t.data_inicio + 'T12:00:00').toLocaleDateString('pt-BR')}</td>
                  <td className="px-6 py-3 text-sm text-gray-400">{new Date(t.data_fim + 'T12:00:00').toLocaleDateString('pt-BR')}</td>
                  <td className="px-6 py-3 text-sm text-green-400">R$ {t.preco_venda}</td>
