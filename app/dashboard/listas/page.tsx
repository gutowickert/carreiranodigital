...
export default function Listas() {
  ...
  const corCat = (c: string) => c === 'comprador' ? 'var(--green)' : 'var(--blue)'
  const corStatus: Record<string, string> = { novo: 'var(--text-muted)', enviado: 'var(--amber)', respondeu: 'var(--green)', virou_lead: 'var(--accent-soft)', optout: 'var(--red)' }

  return (
    <div style={{ padding: '32px clamp(12px, 4vw, 40px)', maxWidth: 1100 }}>
      <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--text)', margin: '0 0 4px' }}>Listas de contatos</h1>
      <p style={{ fontSize: 13, color: 'var(--text-faint)', margin: '0 0 24px' }}>Interessados e compradores antigos pra disparo. Ficam fora do CRM — só viram lead se responderem.</p>

      {/* resumo */}
      {resumo && (
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          <div style={{ ...card, padding: 16, minWidth: 140 }}><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Total</div><div style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)' }}>{resumo.total}</div></div>
          <div style={{ ...card, padding: 16, minWidth: 140 }}><div style={{ fontSize: 12, color: 'var(--blue)' }}>Interessados</div><div style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)' }}>{resumo.interessados}</div></div>
          <div style={{ ...card, padding: 16, minWidth: 140 }}><div style={{ fontSize: 12, color: 'var(--green)' }}>Compradores</div><div style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)' }}>{resumo.compradores}</div></div>
        </div>
      )}

      {/* importar */}
      <div style={{ ...card, padding: 20, marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 12 }}>Importar contatos</div>
        <div style={{ display: 'flex', gap: 16, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ fontSize: 13, color: 'var(--text-2)', cursor: 'pointer' }}><input type="radio" checked={categoria === 'interessado'} onChange={() => setCategoria('interessado')} /> Interessados</label>
          <label style={{ fontSize: 13, color: 'var(--text-2)', cursor: 'pointer' }}><input type="radio" checked={categoria === 'comprador'} onChange={() => setCategoria('comprador')} /> Compradores</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 6 }}>Pode selecionar vários. Lê CSV (Nome/Telefone) e VCF (vCard). VCF não traz cidade — preencha o campo "Cidade" acima antes. Marque a categoria certa!</div>
          </div>
          <div>
        ...
        {resultado && <div style={{ marginTop: 12, fontSize: 12, color: 'var(--green)', whiteSpace: 'pre-wrap', background: 'var(--bg)', borderRadius: 8, padding: 10 }}>{resultado}</div>}
      </div>

      {/* breakdown por cidade */}
      {resumo && resumo.cidades.length > 0 && (
        <div style={{ ...card, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 10 }}>Por cidade</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {resumo.cidades.map(c => (
              <button key={c.cidade} onClick={() => setFCidade(fCidade === c.cidade ? '' : c.cidade)}
                style={{ ...card, padding: '8px 12px', cursor: 'pointer', border: fCidade === c.cidade ? '1px solid var(--accent)' : '1px solid var(--border)', background: fCidade === c.cidade ? 'var(--accent-bg)' : 'var(--bg)' }}>
                <div style={{ fontSize: 13, color: 'var(--text)', fontWeight: 600 }}>{c.cidade}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}><span style={{ color: 'var(--blue)' }}>{c.interessado} int</span> · <span style={{ color: 'var(--green)' }}>{c.comprador} comp</span></div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{ ...card, padding: 16 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12, alignItems: 'center' }}>
          ...
          <button onClick={carregar} style={{ ...btn, background: 'var(--surface-2)' }}>Buscar</button>
          {fCidade && <button onClick={() => setFCidade('')} style={{ ...btn, background: 'var(--surface-2)' }}>Limpar cidade: {fCidade}</button>}
        </div>

        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            <thead>
              <tr style={{ textAlign: 'left', fontSize: 11, color: 'var(--text-faint)', textTransform: 'uppercase' }}>
            ...
            <tbody>
              {contatos.map(c => (
                <tr key={c.id} style={{ borderTop: '1px solid var(--border)', fontSize: 13, color: 'var(--text-2)' }}>
                  ...
                  <td style={{ padding: '8px', color: corStatus[c.status] || 'var(--text-muted)' }}>{c.status}</td>
                  <td style={{ padding: '8px', fontSize: 11, color: 'var(--text-muted)' }}>{c.notas || ''}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 10 }}>{carregando ? 'Carregando...' : `${contatos.length} contato(s) exibido(s) (máx 300 por vez)`}</div>
      </div>
    </div>
  )
}
