    ...
    <Layout>
      <div style={{ padding: '32px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Templates de Tarefas</h1>
        ...
        {novoForm && (
          <div style={{ ...card, padding: 20, marginBottom: 20 }}>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 14, marginTop: 0 }}>
              {editando ? 'Editar template' : 'Novo template'}
            </h3>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Título da tarefa</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <label style={{ display: 'block', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Setor responsável</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Referência</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Quando</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Dias</label>
                <input type="number" min="0" style={{ ...inp, width: '100%' }} value={form.dias} onChange={e => setForm(f => ({ ...f, dias: e.target.value }))} />
              </div>
            </div>

            {!editando && (
              <div style={{ background: 'var(--bg)', padding: 12, borderRadius: 6, marginBottom: 12 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-2)' }}>Aplicar este template em todas as turmas futuras já cadastradas</span>
                </label>
                <p style={{ fontSize: 10, color: 'var(--text-faint)', marginTop: 6, marginLeft: 24 }}>
        ...
        {carregando ? (
          ...
          <div style={{ ...card, padding: 24 }}>
            <p style={{ fontSize: 13, color: 'var(--text-faint)', margin: 0 }}>Nenhum template cadastrado.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  <th style={{ textAlign: 'left', padding: '12px 20px', fontSize: 11, color: 'var(--text-faint)', fontWeight: 500 }}>Tarefa</th>
                  <th style={{ textAlign: 'left', padding: '12px 20px', fontSize: 11, color: 'var(--text-faint)', fontWeight: 500 }}>Setor</th>
                  <th style={{ textAlign: 'left', padding: '12px 20px', fontSize: 11, color: 'var(--text-faint)', fontWeight: 500 }}>Prazo</th>
              ...
              <tbody>
                {templates.map(t => (
                  <tr key={t.id} style={{ borderBottom: '1px solid var(--border)', opacity: t.ativo ? 1 : 0.5 }}>
                    <td style={{ padding: '12px 20px', fontSize: 13, color: 'var(--text)' }}>{t.titulo}</td>
                    <td style={{ padding: '12px 20px', fontSize: 12, color: 'var(--text-muted)' }}>{SETOR_LABEL[t.setor] || t.setor}</td>
                    <td style={{ padding: '12px 20px', fontSize: 12, color: 'var(--text-muted)' }}>{descreverPrazo(t)}</td>
                    <td style={{ padding: '12px 20px', textAlign: 'center' }}>
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4,
                        background: t.ativo ? 'var(--green-bg)' : 'var(--surface-2)',
                        color: t.ativo ? 'var(--green)' : 'var(--text-muted)',
                    ...
                    <td style={{ padding: '12px 20px' }}>
                      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                        <button onClick={() => abrirEdicao(t)} style={{ fontSize: 11, color: 'var(--accent-soft)', background: 'none', border: 'none', cursor: 'pointer' }}>Editar</button>
                        <button onClick={() => alternarAtivo(t)} style={{ fontSize: 11, color: t.ativo ? 'var(--amber)' : 'var(--green)', background: 'none', border: 'none', cursor: 'pointer' }}>
                          {t.ativo ? 'Desativar' : 'Ativar'}
                        </button>
                        <button onClick={() => deletar(t)} style={{ fontSize: 11, color: 'var(--red)', background: 'none', border: 'none', cursor: 'pointer' }}>Deletar</button>
                      </div>
                    </td>
                  </tr>
                ))}
