...
Pontos importantes: DESPESA/custo = lançamentos com tipo 'custo' (a ferramenta financeiro já trata e traduz a NATUREZA). CAMPANHA do anúncio fica em leads.utm_campaign. GASTO de anúncio/tráfego vem ao vivo da Meta (ferramenta trafego, precisa de datas) — não está no banco. MOTIVOS DE PERDA: use a ferramenta 'perdas' (traduz os nomes). LIGAÇÕES: a tabela 'ligacoes' (API4COM) tem as chamadas com o campo metadata.transcricao = o TEXTO do que foi falado na call. Pra saber o que foi conversado por telefone com um lead (ou analisar objeções/argumentos das ligações), use 'consultar' na tabela ligacoes filtrando por lead_id.
