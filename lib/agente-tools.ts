...
export const TOOLS = [
  ...
  { name: 'propor_fluxo', description: 'Propõe MUDAR a CADÊNCIA (etapas+tarefas+prazos) OU a PRIORIDADE da fila/lote — NÃO a comunicação. Use quando o pedido for sobre: número de ligações, quando manda áudio/mensagem, prazo (D+N), ordem, adicionar/remover follow-up (cadência); OU sobre QUEM ATENDER PRIMEIRO, ordem da fila, quantos dias de follow-up entram, priorizar um produto (prioridade). SEMPRE chame fluxo_comercial ANTES pra pegar os handles. NÃO aplica sozinho — vira cartão. (Pra TOM/o que dizer/quando ofertar/objeção, use propor_regra_ia.)', input_schema: { type: 'object', properties: { acao: { type: 'string', description: 'add_tarefa | editar_tarefa | remover_tarefa | editar_regras | editar_prioridade' }, etapa: { type: 'string', description: 'handle da etapa (obrigatório em add/editar/remover tarefa)' }, tarefa: { type: 'string', description: 'handle da tarefa (em editar/remover); ou chave nova (em add)' }, campos: { type: 'object', description: 'tarefa: titulo, dias, acao (ligacao|audio|mensagem|decisao), descricao. PRIORIDADE: followupDias (número), ordemQuente (recente|esperando), ordemFollowup (mais_frio|menos_frio), produtoPrioritario (FC|ANL|null), incluirFaladoHoje (true|false)' }, texto: { type: 'string', description: 'novo texto das regras gerais (só em editar_regras)' } }, required: ['acao'] } },
]
...
export async function runTool(name: string, input: any, origin?: string): Promise<any> {
  ...
  if (name === 'propor_fluxo') {
    ...
    if (!['add_tarefa', 'editar_tarefa', 'remover_tarefa', 'editar_regras', 'editar_prioridade'].includes(acao)) return { erro: 'ação inválida' }
    if (['add_tarefa', 'editar_tarefa', 'remover_tarefa'].includes(acao) && !input?.etapa) return { erro: 'informe a etapa (handle)' }
