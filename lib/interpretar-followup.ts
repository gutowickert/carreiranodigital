...
export async function interpretarFollowup(sb: any, org: string, d: Dossie, lead: LeadInfo, gerar = true): Promise<{ etapa: string; motivo: string; fonte: 'cache' | 'gerado' } | null> {
  ...
  // resumo antigo mas COM etapaReal → usa mesmo assim (melhor que nada; será atualizado quando gerar)
  if (!gerar) return cache?.etapaReal ? { etapa: String(cache.etapaReal), motivo: String(cache.ondeParou || ''), fonte: 'cache' } : null
  // sem resumo ou desatualizado E dentro do orçamento → gera (e cacheia pra próxima ser de graça)
